...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 // Product ordering is handled by the cart system injected into the shop pages.
...
// Responsive testimonial carousel: advances automatically every 3 seconds.
// It pauses while the visitor is interacting with the review area (hover, touch,
// focus or pointer hold), then resumes after the interaction ends.
...
function reviewPages(){return Math.max(1,Math.ceil(reviewItems.length/reviewPerPage()))}
function renderReviews(){
 if(!track||!reviewItems.length)return;
 const per=reviewPerPage(), pages=reviewPages();
 reviewPage=((reviewPage%pages)+pages)%pages;
 reviewItems.forEach((item,i)=>{
   const visible=Math.floor(i/per)===reviewPage;
   item.style.display=visible?'block':'none';
   item.classList.toggle('is-review-visible',visible);
 });
 if(dotsWrap){
   dotsWrap.innerHTML='';
   for(let i=0;i<pages;i++){
     const b=document.createElement('button');
     b.className='review-dot'+(i===reviewPage?' is-active':'');
     b.type='button';
     b.setAttribute('aria-label','Show testimonial group '+(i+1));
     b.addEventListener('click',()=>{reviewPage=i;renderReviews();restartReviewTimer()});
     dotsWrap.appendChild(b);
   }
 }
}
function advanceReviews(){
 if(!reviewHeld && reviewItems.length>1){
   reviewPage=(reviewPage+1)%reviewPages();
   renderReviews();
 }
}
function startReviewTimer(){
 clearInterval(reviewTimer);
 reviewTimer=setInterval(advanceReviews,3000);
}
function restartReviewTimer(){startReviewTimer()}
function holdReviews(){reviewHeld=true;clearInterval(reviewTimer)}
function releaseReviews(){reviewHeld=false;startReviewTimer()}
prev?.addEventListener('click',()=>{reviewPage=(reviewPage-1+reviewPages())%reviewPages();renderReviews();restartReviewTimer()});
next?.addEventListener('click',()=>{reviewPage=(reviewPage+1)%reviewPages();renderReviews();restartReviewTimer()});
window.addEventListener('resize',()=>{renderReviews();restartReviewTimer()});
if(track){
 track.addEventListener('mouseenter',holdReviews);
 track.addEventListener('mouseleave',releaseReviews);
 track.addEventListener('pointerdown',holdReviews);
 track.addEventListener('pointerup',releaseReviews);
 track.addEventListener('touchstart',holdReviews,{passive:true});
 track.addEventListener('touchend',()=>setTimeout(releaseReviews,350),{passive:true});
 track.addEventListener('focusin',holdReviews);
 track.addEventListener('focusout',releaseReviews);
}
renderReviews();
startReviewTimer();
